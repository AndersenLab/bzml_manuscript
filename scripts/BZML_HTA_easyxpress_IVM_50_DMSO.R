...
##################################################
#                                                #
#     BZML  Easyxpress - IVM 500 nm and DMSO     #
#                                                #
##################################################
# drugs in assay: ivermectin (500 nM), DMSO (control)
# all assays combined (1-3)
# DMSO = ivermectin_50 (condition), 0 (concentration_um)

# Define experimental directory and file name 
dirsBZML_IVM50 <- "/projects/b1059/projects/Amanda/projects/cellprofiler-nf/projects/20231225_BZML_HTA_IVM_50/Analysis-20231227" #data and design match
datafileBZML_IVM50 <- "20231225_BZML_HTA_IVM_50_Analysis-20231227.RData"

BZML_IVM50 <- easyXpress::readXpress(
  filedir = dirsBZML_IVM50,
  rdafile = datafileBZML_IVM50,
  design = TRUE,
  doseR = FALSE)

# model selection 
ms_IVM50 <- easyXpress::modelSelection(BZML_IVM50$raw_data)

# flag objects
ef_IVM50 <- edgeOF(data = ms_IVM50)

# Use the clusterOF function
cf_IVM50 <- clusterOF(data = ef_IVM50)

# Plot flagged objects 
c1_IVM50 <- checkOF(data = cf_IVM50, strain, drug)
c1_IVM50

# loop at the plot returned
flagged_plot_IVM50 <- c1_IVM50$p
...
c1_of_strain_IVM50 <- checkOF(
  data = dplyr::filter(cf_IVM50, concentration_um == "0"),
  strain)

c1_of_strain_IVM50plot <- c1_of_strain_IVM50$p
c1_of_strain_IVM50plot 

# save plot 
ggsave(
  filename = glue::glue("{figure_dir}/{today}_flagged_plot_by_strain_DMSO.png"),
  plot = c1_of_strain_IVM50plot,
  width = 10,
  height = 10,
  units = "in",
  dpi = 300)
...
c1_of_strain_IVM50_50 <- checkOF(
  data = dplyr::filter(cf_IVM50, concentration_um == "0.50"),
  strain)

c1_of_strain_IVM50_50plot <- c1_of_strain_IVM50_50$p
c1_of_strain_IVM50_50plot

# save plot 
ggsave(
  filename = glue::glue("{figure_dir}/{today}_flagged_plot_by_strain_IVM50_50.png"),
  plot = c1_of_strain_IVM50_50plot,
  width = 10,
  height = 10,
  units = "in",
  dpi = 300)
...
##                                 ##
#####################################
c2_IVM50 <- checkObjs(data = cf_IVM50, OF = 'filter', strain, drug, concentration_um)

cluster_plot_IVM50 <- c2_IVM50
cluster_plot_IVM50

ggsave(
  filename = glue::glue("{figure_dir}/{today}_size_distribution.png"),
  plot = cluster_plot_IVM50,
  width = 10,
  height = 10,
  units = "in",
  dpi = 300)
...
checkmodels_IVM50 <- cf_IVM50%>%
  mutate(
    i.dir = "/projects/b1059/projects/Amanda/projects/cellprofiler-nf/projects/20231225_BZML_HTA_IVM_50/Analysis-20231227/processed_images",
    w.lab = paste(drug, strain, sep = "_"))

# Now we can run the checkModels function
cm_IVM50_out <- checkModels(data = checkmodels_IVM50 ,
...
uf_IVM50 <- easyXpress::userOF(data = u_IVM50, user)

# Check the object data again to see if the bimodal distributions are resolved.
checkObjs(data = uf_IVM50, OF = "filter", drug, concentration_um)
...
##                                    ##
########################################
cl_IVM50 <- classifierOF(data = uf_IVM50) #11 rows with NAs
...
####################################
# apply the outlierOF function
o_IVM50 <- easyXpress::outlierOF(data = cl_IVM50)
...
checkObjs(data = o_IVM50, OF = 'filter', drug, concentration_um)
...
##           ##
###############
co2_IVM50 <- checkOF(data = o_IVM50, drug, concentration_um) # check object flags again

co2_IVM50$p # check the plot
...
proc.objs_IVM50 <- easyXpress::filterOF(o_IVM50, rmVars = T)
...
raw.wells_IVM50 <- easyXpress::summarizeWells(data = o_IVM50)
...
tf_IVM50 <- easyXpress::titerWF(data = raw.wells_IVM50,
...
n_IVM50 <- easyXpress::nWF(data = tf_IVM50$d, drug, concentration_um, max = 30, min = 5)
...
ow_IVM50 <- easyXpress::outlierWF(data = n_IVM50$d,
...
cw1_IVM50 <- easyXpress::checkWF(data = ow_IVM50, drug, concentration_um)
...
fw_IVM50 <- filterWF(data = ow_IVM50, rmVars = T)
...
ce1_IVM50 <- easyXpress::checkEff(data = fw_IVM50,
                                  drug, strain,
                                  x = concentration_um,
...
reg_IVM50 <- easyXpress::regEff(data = fw_IVM50_clean, #drop
                                drug,
...
del_IVM50 <- easyXpress::delta(data = reg_IVM50$d,
...
checkEff(data = del_IVM50, drug, strain, x = concentration_um,
         y = median_wormlength_um_reg_delta,
         fill = assay_bleach,
         scales = "free_x")

write.csv(del_IVM50, "data/20231228_BZML_del_IVM50_delta_regression.csv", row.names = FALSE)
...
#                           #
#############################
# Box plot and stats for IVM50
# # ANOVA and Tukey HSD for abz
statsIVM50 <- del_IVM50 %>%
  dplyr::filter(drug == "ivermectin_50")%>%
  dplyr::filter(concentration_um == "0.5")%>%
...
HTA.fig.IVM50 <- del_IVM50 %>%  
  ...
  dplyr::filter(concentration_um %in% c("0.5")) %>% 
  ggplot +
  ...
  scale_x_discrete(breaks = c("N2","ECA882", "PHX2647","PHX2502","PHX2481","ECA2687","ECA2688","ECA2689","ECA2690"),
                   labels = c("N2\n \n ", "ben-1\n \n ","avr-14\n \n ","avr-15\n \n ","glc-1\n \n ","avr-14\navr-15\n ","avr-14\nglc-1\n ","avr-15\nglc-1\n ","avr-14\navr-15\nglc-1"))+
  stat_pvalue_manual(statsabz, label = "p.adj.signif",y.position = c(0), xmax="group1", remove.bracket = TRUE)+
  # scale_y_continuous(expand = c(0, 0), limits = c(-600, 0), sec.axis = dup_axis(name = "Ivermectin")) +  # Set y-axis limits
...
HTA.fig.IVM50
ggsave("figures/HTA.fig.IVM50-20231228.png", plot = HTA.fig.IVM50, width = 8, height = 4, units = "in")
ggsave("figures/HTA.fig.IVM50-20231228.svg", plot = HTA.fig.IVM50, width = 8, height = 4, units = "in")
...
HTA.fig.ABZ <- del_ABZ %>%  
  ...
  # scale_x_discrete(breaks = c("N2","ECA882", "PHX2647","PHX2502","PHX2481","ECA2687","ECA2688","ECA2689","ECA2690"),
  #                  labels = c("N2\n \n ", "ben-1\n \n ","avr-14\n \n ","avr-15\n \n ","glc-1\n \n ","avr-14\navr-15\n ","avr-14\nglc-1\n ","avr-15\nglc-1\n ","avr-14\navr-15\nglc-1"))+
  stat_pvalue_manual(statsDMSO, label = "p.adj.signif",y.position = c(0), xmax="group1", remove.bracket = TRUE)+
  scale_y_continuous(sec.axis = dup_axis(name = "Albendazole")) +  # Set y-axis limits
  cowplot::theme_cowplot(12) +
  theme(
...
ggsave("figures/HTA.fig.DMSO_20230831.png", plot = HTA.fig.DMSO, width = 7.5, height = 3.5, units = "in")
...
Fig3 <- cowplot::plot_grid(HTA.fig.ABZ,
                          HTA.fig.IVM25, 
                          ncol = 1,
                          nrow = 2,
                          labels = c("A","B"),
...
ggsave("figures/HTA.ABZ.IVM25.Fig3.png", plot = Fig3, width = 4, height = 4, units = "in")
ggsave("figures/HTA.ABZ.IVM25.Fig3.svg", plot = Fig3, width = 4, height = 4, units = "in")
